...
def wasm_runtime_funcs() -> str:
  ...
  (func $id_list_set (param $L i32) (param $idx i32) (param $v i64)
    ...
    (if (i32.and (i32.ge_s (local.get $idx) (i32.const 0))
                 (i32.lt_s (local.get $idx) (local.get $len)))
      (then
        (local.set $data (i32.load offset=8 (local.get $L)))
  ...
  (func $id_write_all (param $ptr i32) (param $len i32)
    ...
    (block $done
      (loop $again
        ...
        (drop (call $fd_write (i32.const 1) (i32.const {_IOV0_PTR}) (i32.const 1)
                               (i32.const {_NWRITTEN})))
  ...
  (func $id_print (param $s i32)
    (local $n i32)
    (local.set $n (call $id_strlen (local.get $s)))
    (call $id_write_all (local.get $s) (local.get $n))
    (call $id_write_all (i32.const {_NEWLINE_BYTE}) (i32.const 1)))
