...
(`demos/idc_in_id` + `demos/idc_in_id_parse`), not in `idc.py`.
...
- **You know the name.** [`MAP.md`](../demos/idc_in_id_parse/MAP.md) — all 737
...
> Does `demos/idc_in_id{,_parse}` now contain this construct in its own source?
...
| to change | go to |
| --- | --- |
| tokens, keywords, literals | `demos/idc_in_id/chars/` |
...
```sh
IDC_NO_STD=1 tools/parity.sh demos/idc_in_id_parse   # MATCH: emitted C unchanged
...
- **A directory holds 3 entries.** Adding a file may mean adding a directory.
  `find demos/idc_in_id_parse -type d | while read d; do echo "$(ls $d|wc -l) $d"; done | sort -n`
