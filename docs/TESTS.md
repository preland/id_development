...
**What a case writes is not part of its report.** The case's stdout is
discarded, and its stderr is the channel the harness reads the report from:
the runtime's trap line, or the mismatch message. So `eprint` inside a case
writes where the case's `print` does, and nothing a function under test prints
can change, or push out, what its failure says.
...
| --- | ---: | ---: | ---: |
| `id_development` (with editor, idem, kernel) | 5816 | 231 | 5715 |
| `idstd` | 207 | 332 | 59 |
| `c2id` | 1058 | 0 | 1058 |
| `linux_id` | 0 | 0 | 0 |
| **total** | **7081** | **563** | **6832** |

**Functions short of two cases: 6832 of 7081 (3.5% complete).** Generated by
