...
| **B3** | ~~`--target llvm` / `--target wasm` are `idc/idc.py`-only.~~ Both are self-hosted now. `--target llvm` lowers to LLVM IR (docs/LLVM.md). `--target wasm` (docs/TODO.md 9b) is not a new AST walk or IR: it is the C target's own emitted C, recompiled for `wasm32-wasi` by an unwrapped clang and linked as a WASI reactor (`crt1-reactor.o`, `--no-entry`, `--gc-sections`), exporting only `memory`, `_initialize`, `id_alloc`, and the program's own top-level functions -- idstd and the runtime are excluded by path before the export list is built, and `--gc-sections` drops their now-unreferenced code from the module too. Needs the umbrella's `nix develop` shell (`IDC_WASI_CLANG`/`IDC_WASI_SYSROOT_INCLUDE`/`IDC_WASI_LIBDIR`/`IDC_WASI_COMPILER_RT`, flake.nix). `idc/idc.py`'s own `WasmBackend` (idc.py:3316-4610) is untouched and still exists, but is no longer the only path to a `.wasm`. | `idc/bin/idc idc/tests/fixtures/... --target wasm -o x.wasm` → wasm-objdump -x -j Export shows exactly `memory`, `id_alloc`, `_initialize`, and one export per top-level function; `node` loads it against the WASI import set in `id-runtime.ts`. |
...
| **C8** | **A conf.id list constant's mutation trap does not reach `--freestanding`.** Hosted (C and LLVM), mutating a list constant traps with `id: cannot mutate a constant list` (`docs/SPEC.md` §8). The freestanding runtime (`idc/runtime`) cannot: it has no stderr and nothing to exit to, the same reason `list_get`/`list_set` there do not bounds-check (`idc/runtime/lang/list/use/more/idx.id`). `idc/runtime`'s `list_lock` is a no-op for this reason, kept only so freestanding builds still link against the call `idc_const_init` makes on every target; a kernel program that mutates a list constant, directly or through an alias, does so silently. Options if this is revisited: give the freestanding runtime a fault-based trap the way `sdiv`/`smod` get one from `idivq` raising `#DE` (`idc/runtime/lang/num/div/sgn.id`) -- which would need the list constant's page write-protected, i.e. paging, which `idc/runtime` does not set up; or accept the silent gap and document it here, which is what this entry does. |  `idc/runtime/lang/list/use/tail.id`'s `list_lock`. |
