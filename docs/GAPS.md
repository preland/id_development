# What still stands between `bin/idc` and being the only compiler
...
`bin/idc` is the primary compiler. `idc.py` is the reference implementation and
the bootstrap: it builds `idlex`/`idparse` once on a cold cache, and that is
meant to be its only remaining job.

It is not, today. `idc.py` is still load-bearing for three things a real program
...
### Tier A — `bin/idc` cannot yet replace `idc.py`

| # | issue | verified how |
| --- | --- | --- |
| **A1** | **No `extern` declarations for link-time-resolved calls, so no program with a native backend builds.** `idc.py` collects unresolved call names and emits `extern int id_<name>();` (idc.py:1195); `idparse` emits nothing, and `bin/idc`'s `cc -fsyntax-only` gate then rejects its own output as an "internal error". | `bin/idc demos/gfxdemo --backend backends/gfx` → `implicit declaration of function 'id_gfx_present'` ×4, then `idc: internal error: the self-hosted compiler emitted C that does not compile`. Same for `nativeapp/id`, `demos/gl3d`. |
...
This is the single most damaging item, because `bin/idc` does not merely fail —
it asserts *"this is a bug in the self-hosted compiler; please report it"* about
the user's own type error. `tests/invalid.sh` does not catch it because it runs
`idc.py` only.
...
| --- | --- |
| "the self-hosted lexer still mis-lexes float literals (`0.8` → `0` `.` `8`); treat `float` as unavailable" (`id-patterns.md` §0) | **FIXED.** `float a = 1.5; print(a * 2.0 + 0.25)` builds and runs under `bin/idc`. |
| "no type errors at all — dumps raw `cc` output" (`id-language.md` §0) | **Partly fixed.** Declaration/assignment types, `push` element type, index type, comparison, `void[]`, empty-list literal, condition types are all checked in `id` now, with `file:line: error:` text identical to `idc.py`'s. The gap is now specifically *call sites and unary/builtin operands* (A2, A3). |
...
| "`bin/idc` transparently falls back to `idc.py`" (README, and the report quoting it) | **Removed** (commit `edb5baf`). The README still describes the fallback and the float gap — see B5. |

### Tier B — driver (`bin/idc`) disagrees with `idc.py`
...
These are not `bin/idc` bugs; they are `id`'s, and the report's own §"Silent
...
| **C5** | Exported names become **unprefixed C globals**, so `export int time` / `stdout` / `index` collide with libc and fail in generated code. | report `id-language.md` §11; mechanism confirmed in `idc.py` codegen. |
...
| **D3** ✅ | **`backends/gfx` cannot resize or scale.** No `StructureNotifyMask`, so `ConfigureNotify` never arrives; `XPutImage` blits 1:1; there is no `gfx_width()`/`gfx_height()`. Under a tiling WM the image sits in the top-left corner with black margins. `backends/gl` already does all of this correctly. | |
...
| **D5** ✅ | **`backends/gl` has no `glReadPixels`**, so there is no headless verification path for GPU output — the software path has PPM dumps, the GL path has nothing. | |
...
| **E2** ✅ | **`demos/gfxdemo/loop/anim/grid.id:2` states the runtime "drops any out-of-range store, so no per-pixel bounds check is needed". That is false** — an out-of-range list store is a fatal abort. Its box merely happens to stay in bounds. Anyone who trusts that comment writes a rasteriser that dies on the first off-screen pixel. | `grid.id:2`; `tests/runtime_invalid/` has `store_oob` proving the abort. |
...
**The count that mattered:** of the 38 canonical invalid programs, `bin/idc`
gave the wrong message for 15. It now gives the expected message for all 40
(two cases were added), and so does `idc.py` — `tests/invalid.sh` runs both and
requires both, which is the change that keeps this from happening again.

## 2. The plan

Ordered so that each phase ends with `bin/idc` strictly more capable and the
parity gate still green. Phases 1-3 are the ones that matter; 4-7 are cleanup
and can be reordered freely.

### Phase 1 — make `bin/idc` build everything `idc.py` builds  *(unblocks A1, A3)*

The extern gap and the unknown-function gap are **one mechanism**, exactly as in
`idc.py`: a call whose name resolves to nothing is an error when no backend is
...
`tools/devshell.sh`). `bin/idc x.id` with a typo'd builtin gives `idc.py`'s
message. `tools/parity.sh` still MATCH on every demo. Add
`tests/self_host_build.sh` cases for a backend build and for the typo message.
...
**Validation — this is the important half.** `tests/invalid.sh` currently runs
`idc.py` only, which is exactly why 15 broken messages went unnoticed. Change it
...
`bin/idc`-vs-expected count must be 40/40, and `compiler_bug()` must be
...
Small, independent, all in `bin/idc` and `README.md`.
...
4. **Rewrite README §"Self-hosting" and §"`bin/idc`: the self-hosted driver"**
   to describe what is true now: no fallback, floats supported, and a single
   honest table of what `idc.py` alone still does (which by then is
...
Each needs a `tests/invalid/` case and, for the runtime ones, a
`tests/runtime_invalid/` case.
...
2. `D3` gfx resize: `StructureNotifyMask`, reallocate `g_px`/`XImage` on
   ...
   its framebuffer.
3. `D4` rename `backends/gl`'s three window entry points to `glwin_*` so both
...
| --- | --- | --- |
| **C1** | A statement whose whole expression is an equality is rejected. `=` and `==` are one operator by the time either parser is done, so both spellings are caught -- which is the right net anyway: `x == 2;` as a statement is as pointless as the assignment it is usually a typo for. An index target gets told to use `lset`. | `tests/invalid/discarded_comparison.id` |
| **C2** | `int` `/` and `%` go through checked helpers, exactly as `word` already did. gcc folds the check away whenever the divisor is a nonzero constant, and `-7/2` is still `-3`. | `tests/runtime_invalid/int_div_zero.id`, `int_mod_zero.id` |
| **C3** | A **reachable** `(import x)` whose exporting function is unreachable from `main` is rejected. Reported at the read, not at the dead function: a file that declares an export and never wires it up is unfinished or illustrative -- `demos/hello` is deliberately the latter -- while a reachable read of an uninitialised global is neither. Verified against the bug it was designed for: with `demos/fpsmaze`'s fix reverted, both compilers catch it. | `tests/invalid/dead_export.id` |
| **C4** | Both lexers recognise `/*` and say "block comments are not supported; use // for a line comment". The `id` lexer consumes the comment and emits one token, so one comment is one diagnostic instead of a page of complaints about the words inside it. | `tests/invalid/block_comment.id` |
| **+** | Bonus, found on the way: an operator where an expression belongs used to be turned into a *variable named after the punctuation* and reported by a later pass as `undefined variable '/'`. Both compilers now say `unexpected token '/'`. | `tests/invalid/unexpected_operator.id` |
| **+** | Bonus, found by the input probe: `idc.py` types a link-time call `int`, the self-hosted compiler left it unknown -- so `"" + gfx_width()` emitted a raw int where a `char*` belonged. A real parity divergence, only reachable once backends built at all. | the backend parity checks |
...
| **D3** | `backends/gfx` follows `ConfigureNotify`, reallocates the surface, and exposes `gfx_width()`/`gfx_height()`. `id` had **no way** to learn the real window size before. | verified: a window resized from 320×200 to 922×2020 and `id` saw it |
| **D4** | The GL backend's window entry points are `glwin_open/poll/close`. They used to be `gfx_*` -- the same symbols the software backend exports -- so the two could not be linked together at all. | `tests/backends.sh` links both into one binary and drives two windows |
| **D5** | `gl_read_pixels(fb)` reads the rendered frame back as `0xRRGGBB`, top row first -- the same layout `gfx_present` consumes, so one PPM dumper serves both paths. Reads `GL_BACK` before the swap: reading `GL_FRONT` after it looks more natural and returns black under a compositor. | `tests/backends.sh` renders a known colour and reads it back from `id` |
| **E1** | Six demos, not one: `gfxdemo`, `fpsmaze`, `gl3d`, `gl3dgame`, `flyover`, `galaxy` all ignored `gfx_open`'s result and ran until killed under `DISPLAY=`. | `tests/backends.sh` runs each with no display |
...
**A note on `idc.py`.** Four of these -- C1, C2, C3, C4 -- are new diagnostics,
...
get, and that is the exact hole `tests/invalid.sh` was changed to close. The
runtime lives in `idc.py` as one string that both compilers emit verbatim
(`tools/gen_runtime_id.py` regenerates the `id`-side copy), so parity proves
they agree rather than requiring them to be kept in step by hand.

## 3. Performance: what was actually slow

A6 said "33× slower than `idc.py`, and superlinear". Both halves were true, and
...
| ratio to `idc.py` at 800 files | — | **2.2×** |
...
1. **`id_charat` called `strlen` on every character access** — so walking a
   ...
   This is a **runtime** fix, in `idc.py`'s `RUNTIME` with the `id`-side copy
   regenerated by `tools/gen_runtime_id.py`, so both compilers emit it and
...
`bin/idc` is the only compiler a user ever runs when all of these hold:

- ✅ Every program in `demos/`, `nativeapp/`, and `backends/`-using trees builds
  with `bin/idc`, including native backends.
- ✅ All 40 cases in `tests/invalid/` produce their expected message under
  **both** compilers, and `compiler_bug()` fires only for a genuine compiler
  bug.
- ✅ `bin/idc` is within ~3× of `idc.py` at realistic project shapes (2.2× at
  ...
  in §3.
- ✅ `tools/parity.sh` is MATCH — checked on every demo, `nativeapp/id`, and
  both graphics backends.
- ✅ Every diagnostic either compiler gives, both give. `tests/invalid.sh` and
  `tests/runtime_invalid.sh` both run both.
- ⬜ `idc.py`'s only remaining job is `--target llvm|wasm`. Bootstrapping is
