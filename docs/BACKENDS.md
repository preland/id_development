...
`sys/io/fs`, `sys/win/gfx` and `sys/win/gl`). A second compiler target must not
drag the second kind along with it.
...
`sys/win/gfx`, `sys/win/gl`. A program calls a library function (or, until the
